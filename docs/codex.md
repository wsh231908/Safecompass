...
- 实现了攻击策略层，实现了一些攻击策略。
- 我在本地部署了Llama-3-70b的一个压缩精度的模型允许使用。

我准备要实现的内容：

- 丰富数据可视化的内容
- 我希望实现的效果是这样的：一个平台，允许用户使用jailbreak bench的数据集，或者harmbench数据集，或者自己上传自定义数据集。然后可以通过我的攻击策略层，生成攻击prompt，给到大模型进行评测。最后生成一个对于这个模型的ASR效果，最好还能生成该模型的能力维度图。
